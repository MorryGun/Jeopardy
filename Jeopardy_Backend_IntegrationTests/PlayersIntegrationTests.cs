...
namespace Jeopardy_Backend_IntegrationTests.PlayersIntegrationTests
{
    ...
    public class PlayersIntegrationTests : BaseIntegrationTests
    {
        ...
        private const string PlayerPath = "api/player";
        ...
        {
            // Act
            var response = await this.client.GetAsync(PlayerPath);
        ...
        public async Task PostPlayers_Returns_UnauthorizedForUnauthorizedUser()
        {
            ...
            // Act
            var response = await this.client.PostAsync(PlayerPath, content);
        ...
        public async Task PostPlayers_Adds_Player()
        {
            ...
            var initialPlayersCount = await this.GetItemsCount<Player>(PlayerPath, this.client);
            ...
            // Act
            var response = await authorizedClient.PostAsync(PlayerPath, content);

            // Assert
            Assert.Equal(HttpStatusCode.OK, response.StatusCode);

            var actualPlayersCount = await this.GetItemsCount<Player>(PlayerPath, this.client);
        ...
        public async Task PostPlayer_ReturnsPlayerCreated()
        {
            ...
            Assert.NotNull((await this.GetItems<Player>(PlayerPath, this.client)).FirstOrDefault(x => x.Name == name));
        }
        ...
        public async Task UpdatePlayer_UpdatesPlayerCreated()
        {
            ...
            var player = (await this.GetItems<Player>(PlayerPath, this.client)).FirstOrDefault(x => x.Name == name);
            player.Name = updatedName;

            var content = this.CreateContent(player);

            // Act
            var response = await authorizedClient.PutAsync(PlayerPath + "/" + player.Id, content);

            // Assert
            Assert.Equal(HttpStatusCode.OK, response.StatusCode);

            var updatedPlayer = (await this.GetItems<Player>(PlayerPath, this.client)).FirstOrDefault(x => x.Name == updatedName);
        ...
        private async Task AddPlayer(string name, HttpClient client)
        {
            var body = new Player() { Name = name };
            var content = this.CreateContent(body);
            await client.PostAsync(PlayerPath, content);
        }
    }
}
